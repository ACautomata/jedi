...
from jedi.models import CrossModalityJEPA, MLP, ModalityEmbedder, ViT3DEncoder
...
class TestEncoderStep(unittest.TestCase):
    ...
    def test_stage1_step_with_modality_conditioning(self):
        ...
        predictor = LatentPredictor(input_dim=64, hidden_dim=64)
        pred_proj = MLP(input_dim=64, hidden_dim=128, output_dim=64, norm_fn="LayerNorm")
        modality_embedder = ModalityEmbedder(num_modalities=4, emb_dim=64)
        ...
        batch = {
            "src": torch.randn(2, 1, 32, 32, 32),
            "tgt": torch.randn(2, 1, 32, 32, 32),
            "tgt_modality_idx": torch.tensor([1, 3]),
        }
        loss = module.training_step(batch, 0)
        self.assertTrue(torch.isfinite(loss["loss"]))
